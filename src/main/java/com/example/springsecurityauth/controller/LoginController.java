...
public class LoginController {
	...
	@GetMapping("/")
	public String getUserInfo(Principal principal, @AuthenticationPrincipal OidcUser oidcUser) {
		StringBuffer userInfo = new StringBuffer();
		if (principal instanceof UsernamePasswordAuthenticationToken) {
			userInfo.append(getUsernamePasswordLoginInfo(principal));
		} else if (principal instanceof OAuth2AuthenticationToken) {
			userInfo.append(getOauth2LoginInfo(principal, oidcUser));
		}
	...
	private StringBuffer getOauth2LoginInfo(Principal user,OidcUser oidcUser) {
		   ...
		   // key login pour authToken du fournisseur d identité  Github et key "name" pour du fournisseur d identité  console google cloud
